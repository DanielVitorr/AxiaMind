...
import { FlatList, KeyboardAvoidingView, ScrollView, StatusBar, Text, TextInput, TouchableOpacity, View, VirtualizedList } from "react-native"
...
export default function RegistroSaida() {
  ...
  const registrarEntreda = async () => {
    if (!valorReal || !selectCategoria || !selectDateVencimento || !selectDatePagamento || !selectStatusPago || !selectTipoPagamento) {
      alert('Preencher todos os campos!')
      return
    }

    try {
      const valorNumero = Number(valorReal.replace(/[^0-9,-]+/g, "").replace(',', '.'))

      await FinancasDataBase.createSaidas({
        titulo: titulo,
        valor: valorNumero,
      ...
      <View style={style.container}>
        ...
        <View style={style.content}>
          ...
          <View style={style.contentInputs}>
            ...
            <TextInput style={style.inputTexto} placeholder="Titulo" value={titulo} onChangeText={setTitulo} />
          </View>
          ...
          <View style={style.contentInputs}>
            ...
            <View style={style.selectList}>
              ...
              >
                <Picker.Item label='Selecinar' value={null} style={style.selectCategoriaTexto} />
          ...
          <View style={style.contentInputs}>
            ...
            <View style={style.selectDataRecebimento}>
              <TouchableOpacity
                style={[
                  style.selectButton,
                  selectDateVencimento === dayjs().format('YYYY-MM-DD') && style.selectButtonAtivo
                ]}
                onPress={() => handleSelectDayVencimento('hoje')}
              >
                <Text style={[
                  style.selectButtonText,
                  selectDateVencimento === dayjs().format('YYYY-MM-DD') && style.selectButtonTextAtivo
                ]}>Hoje</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
                  style.selectButton,
                  selectDateVencimento === dayjs().subtract(1, 'day').format('YYYY-MM-DD') && style.selectButtonAtivo
                ]}
                onPress={() => handleSelectDayVencimento('ontem')}
              >
                <Text style={[
                  style.selectButtonText,
                  selectDateVencimento === dayjs().subtract(1, 'day').format('YYYY-MM-DD') && style.selectButtonTextAtivo
                ]}>Ontem</Text>
              </TouchableOpacity>
              <TouchableOpacity style={style.selectButton} onPress={handleShowCalendarVencimento}>
                <Text style={style.selectButtonText}>
                  {calendarDateVencimento ? dayjs(calendarDateVencimento).format("DD/MMMM/YYYY") : "Selecione uma data"}
                </Text>
              </TouchableOpacity>
            </View>
          ...
          <View style={style.contentInputs}>
            ...
            <View style={style.selectDataRecebimento}>
              <TouchableOpacity
                style={[
                  style.selectButton,
                  selectDatePagamento === dayjs().format('YYYY-MM-DD') && style.selectButtonAtivo
                ]}
                onPress={() => handleSelectDayPagamento('hoje')}
              >
                <Text style={[
                  style.selectButtonText,
                  selectDatePagamento === dayjs().format('YYYY-MM-DD') && style.selectButtonTextAtivo
                ]}>Hoje</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
                  style.selectButton,
                  selectDatePagamento === dayjs().subtract(1, 'day').format('YYYY-MM-DD') && style.selectButtonAtivo
                ]}
                onPress={() => handleSelectDayPagamento('ontem')}
              >
                <Text style={[
                  style.selectButtonText,
                  selectDatePagamento === dayjs().subtract(1, 'day').format('YYYY-MM-DD') && style.selectButtonTextAtivo
                ]}>Ontem</Text>
              </TouchableOpacity>
              <TouchableOpacity style={style.selectButton} onPress={handleShowCalendarPagamento}>
                <Text style={style.selectButtonText}>
                  {calendarDatePagamento ? dayjs(calendarDatePagamento).format("DD/MMMM/YYYY") : "Selecione uma data"}
                </Text>
              </TouchableOpacity>
            </View>
          ...
          <View style={style.contentInputs}>
            ...
            <View style={style.statusPago}>
              <TouchableOpacity
                style={[
                  selectStatusPago === 1 ? style.selectButtonStatusPagoActive : style.selectButtonStatusPago
                ]}
                onPress={() => handleSelectStatusPago('pago')}
              >
                <Text style={[
                  selectStatusPago === 1 ? style.selectButtonStatusTextoActive : style.selectButtonStatusTexto
                ]}>Pago</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
                  selectStatusPago === 0 ? style.selectButtonStatusNaoPagoActive : style.selectButtonStatusNaoPago
                ]}
                onPress={() => handleSelectStatusPago('naoPago')}
              >
                <Text style={[
                  selectStatusPago === 0 ? style.selectButtonStatusTextoActive : style.selectButtonStatusTexto
                ]}>Não Pago</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={style.contentInputs}>
            ...
            <View style={style.contentTipoLancamento}>
              <TouchableOpacity
                style={[
                  ...
                  selectTipoPagamento === 'Unico' && style.selectButtonTipLancUniAtivo
                ]}
                onPress={() => handleSelectTipoPagamento('Unico')}
              >
                <Text style={[
                  style.selectButtonTipoLancUniText,
                  selectTipoPagamento === 'Unico' && style.selectButtonTipLancUniTextAtivo
                ]}>Único</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
                  style.selectButtonTipLancParc,
                  selectTipoPagamento === 'Parcelado' && style.selectButtonTipLancParcAtivo
                ]}
                onPress={() => handleSelectTipoPagamento('Parcelado')}
              >
                <Text style={[
                  style.selectButtonTipLancParcText,
                  selectTipoPagamento === 'Parcelado' && style.selectButtonTipLancParcTextAtivo
                ]}>Parcelado</Text>
              </TouchableOpacity>
            </View>
            {selectTipoPagamento === 'Parcelado' && (
              <>
                <View style={style.contentInputs}>
                  <Text style={style.contentTitulo}>Quantidade de Parcelas</Text>
                  <View style={style.selectList}>
                    <Picker
                      selectedValue={selectTipoPagamentoQuant}
                      onValueChange={(itemValue) => setSelectTipoPagamentoQuant(itemValue)}
                    >
                      <Picker.Item label='Selecione' value={null} style={style.selectCategoriaTexto} />
                ...
                <View style={style.contentInputs}>
                  ...
                  <View style={style.selectList}>
                    <Picker
                      ...
                      onValueChange={(itemValue) => setSelectTipoParcelamento(itemValue)}
                    >
                      <Picker.Item label='Selecionar' value={null} style={style.selectCategoriaTexto} />
                      <Picker.Item label='Mensal' value='Mensal' style={style.selectCategoriaTexto} />
                      <Picker.Item label='Quinzenal' value='Quinzenal' style={style.selectCategoriaTexto} />
                      <Picker.Item label='Semanal' value='Semanal' style={style.selectCategoriaTexto} />
                    </Picker>
                  </View>
                </View>
              </>
        ...
        {/* -------------------- Adicionar -------------------- */}
        <TouchableOpacity style={style.botaoAdicionar} onPress={registrarEntreda}>

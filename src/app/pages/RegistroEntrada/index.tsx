...
export default function RegistroEntrada() {
  ...
  const [dataRecebimento, setDataPagamento] = useState("")
  ...
  const handleDayPress = (selectedDay: DateData) => {
    setDay(selectedDay)
    setSelectDate(selectedDay.dateString)
  ...
  const registrarEntreda = async () => {
    if (!valorReal || !selectCategoria || !selectDate) {
    ...
    try {
      ...
      await FinancasDataBase.createEntradas({
        titulo: titulo,
        valor: valorNumero,
        categoria: selectCategoria,
        dataRecebimento: selectDate,
            ...
            <View style={style.inputs}>
              ...
              <View style={style.selectCategoria}>
                <Picker
                  ...
                  onValueChange={(itemValue, intemIndex) => setSelectCategoria(itemValue)}
                >
                  <Picker.Item label='Selecinar' value='' style={style.selectCategoriaTexto} />
            ...
            <View style={style.containerDataRecebimento}>
              ...
              <View style={style.selectDataRecebimento}>
                <TouchableOpacity
                  style={[
                    style.selectButton,
                    selectDate === dayjs().format('YYYY-MM-DD') && style.selectButtonAtivo
                  ]}
                  onPress={() => handleSelect('hoje')}
                >
                  <Text style={[
                    style.selectButtonText,
                    selectDate === dayjs().format('YYYY-MM-DD') && style.selectButtonTextAtivo
                  ]}>Hoje</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    style.selectButton,
                    selectDate === dayjs().subtract(1, 'day').format('YYYY-MM-DD') && style.selectButtonAtivo
                  ]}
                  onPress={() => handleSelect('ontem')}
                >
                  <Text style={[
                    style.selectButtonText,
                    selectDate === dayjs().subtract(1, 'day').format('YYYY-MM-DD') && style.selectButtonTextAtivo
                  ]}>Ontem</Text>
                </TouchableOpacity>
                <TouchableOpacity style={style.selectButton} onPress={handleShowCalendar}>
                  <Text style={style.selectButtonText}>
                    {calendarDate ? dayjs(calendarDate).format("DD/MM/YYYY") : "Selecione uma data"}
                  </Text>
                </TouchableOpacity>
              </View>
              {showCalendar && (
